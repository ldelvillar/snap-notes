...
async function fillAndSubmit(overrides: FormOverrides = {}) {
  ...
  if (password) await user.type(screen.getByPlaceholderText(/8\+ chars/i), password);
  if (repeatPassword) await user.type(screen.getByPlaceholderText(/repeat password/i), repeatPassword);
